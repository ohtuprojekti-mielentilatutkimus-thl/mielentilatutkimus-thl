...
test('Link to admission form is sent', async () => {
    Mailer.sendLinkToAdmissionForm(helper.basicInfoFormTestData.email, '616014ea275e2df3c553b958')
    await new Promise((t) => setTimeout(t, 1000))
    maildev.getAllEmail(function (err, emails) {
        expect(err).toBeNull()
        expect(emails.length).toBe(1)
        email = emails[0]
        expect(email.to).toStrictEqual([{ address: helper.basicInfoFormTestData.email, name: '' }])
...
test('Link to editing admission form is sent', async () => {
    ...
    Mailer.requestAdditionalInfoFromSender(helper.basicInfoFormTestData.email, '616014ea275e2df3c553b958', 'henkilötunnus puutteellinen')
...
test('Confirmation is sent', async () => {
    Mailer.sendConfirmation(helper.basicInfoFormTestData.email, helper.admissionFormTestData.diaariNumber, '616014ea275e2df3c553b958')
    await new Promise((t) => setTimeout(t, 1000))
    maildev.getAllEmail(function (err, emails) {
        expect(err).toBeNull()
        expect(emails.length).toBe(1)
